...
const useImagePicker = () => {
  ...
  const compressImageIfNeeded = useCallback(async (uri) => {
    const info = await File.getInfoAsync(uri);
    if (info.size > MAX_IMAGE_SIZE) {
      ...
      const compressedInfo = await File.getInfoAsync(compressed.uri);
  ...
  const validateFile = useCallback(async (asset) => {
    const info = await File.getInfoAsync(asset.uri);
  ...
  }, [App_Language, compressImageIfNeeded]);

  // Pick images or videos
  const pickImage = useCallback(
    async ({ multiple = false, maxFiles = 10, allowVideo = false } = {}) => {
      try {
